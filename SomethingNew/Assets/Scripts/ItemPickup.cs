...
public class ItemPickup : MonoBehaviour
{
    ...
    public GameObject lightningButton; //Public game object variable called ligtningButton
    ...
    void OnTriggerEnter2D(Collider2D other) //Method with a parameter for a 2D Collider object
    {
        if (other.CompareTag("Player")) //Check if other has the 'player' tag
        {
            for (int i = 0; i < inventory.slots.Length; i++) //For loop that runs through the slots array, stopping when i is not less than the slots array length
            {
                if (inventory.isFull[i] == false) //Checks if the slot we are currently on is empty
                {
                    ...
                    Instantiate(lightningButton, inventory.slots[i].transform, false); //Creates a clone of lightningButton, parents it to the slot chosen which also puts it in the same position as the slot, specifies not in world space
